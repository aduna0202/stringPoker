<template>
    <div class="cashDisplays">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 80">
            ...
            <rect style="fill:#231f20; stroke:#5b5b5b; stroke-miterlimit:10;"  @click="updateBet()" :class="{glow:glow && !lockBet, noGlow:!glow}" x="46" y="12.5" rx="2" width="53.5" height="10" />
            ...
            <text text-anchor="end" font-weight="bold" font-size="9" x="43" y="20.5" fill="#FFFFFF" opacity="0.8">
                BET</text>
            <text text-anchor="middle" font-weight="bold" font-size="9" x="73" y="20.5" fill="#FFFFFF" opacity="1" :class="{pointer: glow && !lockBet}" @click="updateBet()">
                {{dollarFormat(cash.coinValue * cash.base_coin_cost)}} + {{dollarFormat(cash.coinValue * cash.bonusCost)}}</text>
  ...
  methods: {
    ...
    //   console.log(this.cash, this.cash.win);
      if (this.cash.win > 0) {
       ...
       /*      if (this.cash.win >= ((this.cash.coinValue * this.cash.base_coin_cost) + (this.cash.coinValue * this.cash.bonusCost))) {
              console.log('win??');
        ...
        //  console.log(this.winDisplayed,  this.cash.win, (this.cash.coinValue * this.cash.base_coin_cost) + (this.cash.coinValue * this.cash.bonusCost));
        }, 30);
      } else {
        this.winDisplayed = 0;
 //console.log('lose??');
