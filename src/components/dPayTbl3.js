...
function tbl(value, coinValue, id, payVersion) {
    ...
    colSpecs.forEach((c, i) => {
        ...
        col.enter().append('text')
            ...
            /*     if (d.bonusReward) {
                ...
                return d.bonusReward[payVersion]

              /*   var bonusIndex = bonusValues.map((d) => { return d.text }).indexOf(d.text);
                if (bonusIndex > -1) {
                    return bonusValues[bonusIndex]['reward' + payVersion] + '*     ' + commaValue(d.reward * coinValue);
                } else {
                    return commaValue(d.reward * coinValue)
                } */      
            });
